...
// Function to checksum IBAN
// Taken from https://stackoverflow.com/a/44657292
 
function validateIbanChecksum(iban) { 
 const ibanStripped = iban.replace(/[^A-Z0-9]+/gi,'') //keep numbers and letters only
 .toUpperCase(); //calculation expects upper-case
if(ibanStripped.length !== 22) return false;
 const m = ibanStripped.match(/^(DE)([0-9]{2})([0-9]{9,30})$/);
 if(!m) return false;
